...
int str_redis_add(redisContext *conn, char *key, char *value)
{
	int res = 0;
	redisReply *reply = NULL;
...
int str_redis_upd(redisContext *conn, char *key, char *value)
{
	int res = 0;
...
int str_redis_get(redisContext *conn, char *key, char *value, int *len)
{
	int res = 0;
	redisReply *reply = NULL;
	...
	if(strlen(reply->str) > *len-1)
	{
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "string lenght error input:%d out:%d\n", *len, strlen(reply->str));
		res = -1;
		goto END;
	}
